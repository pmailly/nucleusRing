...
stat_summary(fun.data = mean_se, fun.args = list(mult = 1),
        geom = 'errorbar', color = 'red', width = 0.5) + stat_summary(fun = mean, geom = 'point', color = 'red') + xlab('Condition') + ylab('ring width (µm)')
...
# read peak's ratio
df <- read.csv(file = file.path(outDir, "peaksStats.csv", fsep = dirSep), sep = ',', header = TRUE)
p <- ggplot(df, aes(x = Condition, y = df$Ratio.ring_nucleus.)) + geom_dotplot(binaxis = 'y', stackdir = 'center', dotsize = 0.5) +
stat_summary(fun.data = mean_se, fun.args = list(mult = 1), 
